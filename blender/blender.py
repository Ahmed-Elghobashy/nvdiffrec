...
# path to your mesh
MESH_PATH = "../out/ch_locked_train_lock_lighting_no_learn_light_ks_2.0_radius_30_degree_center_position/mesh"
...
import bpy.utils
for sp in bpy.utils.script_paths():
    addons_dir = os.path.join(sp, "addons")
    if os.path.isdir(addons_dir) and addons_dir not in sys.path:
        sys.path.append(addons_dir)

# 2) Enable the add-on in this session via the Preferences API:
try:
    bpy.ops.preferences.addon_enable(module="io_scene_obj")
except Exception:
    pass

# 3) As a fallback, import and register it directly:
if not hasattr(bpy.ops.import_scene, "obj"):
    mod = importlib.import_module("io_scene_obj")
